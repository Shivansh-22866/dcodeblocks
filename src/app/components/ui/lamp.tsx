...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[20rem] flex-col py-0 -my-10 items-center justify-center overflow-hidden bg-transparent w-full rounded-md z-0",
        className
      )}
    >
      <div className="relative flex w-full flex-1 translate-y-[8rem] scale-y-125 h-fit items-center justify-center isolate z-0 ">
        <motion.div
        ...
        >
          <div className="absolute  w-[100%] left-0 bg-slate-950/50 h-40 bottom-0 z-20 [mask-image:linear-gradient(to_top,white,transparent)]" />
          <div className="absolute  w-40 h-[100%] left-0 bg-slate-950/40  bottom-0 z-20 [mask-image:linear-gradient(to_right,white,transparent)]" />
        </motion.div>
        <motion.div
          initial={{ opacity: 0.5, width: "15rem" }}
          whileInView={{ opacity: 1, width: "30rem" }}
      ...
      <div className="relative z-50 flex flex-col -translate-y-10 items-center px-5">
        {children}
      </div>
    </div>
  );
